...
p_score = dill.dumps(all_phred)
cctx = zstandard.ZstdCompressor(level=11, threads=-1)
p_seq_c = cctx.compress(p_seq)
p_score_c = cctx.compress(p_score)

# compute dimensions of image for seq and score
dim = math.floor(math.sqrt(len(p_score_c))) + 1
target = dim * dim
diff = target - len(p_score_c)
n_score = p_score_c + bytes(int(diff))
diff_seq = target - len(p_seq_c)
n_seq = p_seq_c + bytes(int(diff_seq))

# save data to separate images
img_score = Image.frombytes('P', (int(dim), int(dim)), n_score)
...
maskmap.save("ERR6099228_maskmap_RGB.webp", lossless=True, bits=8)
